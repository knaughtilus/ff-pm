<?php
    ...
    $projects = array_slice(scandir(getcwd()), 2);
    ...
    //Remove if in excludedDirs
    foreach($excludedDirs as $excl) {
        unset($projects[array_search($excl, $projects)]);
    }

    //Remove if yaml already exists
    foreach($YAMLfiles as $excl) {
        unset($projects[array_search(basename($excl, ".yaml"), $projects)]);
    }

    //Remove if not a directory
    foreach($projects as $file) {
        if (is_file($file)) {
            unset($projects[array_search($file, $projects)]);
        }
    }

    if(count($projects) === 0) {
        setcookie("warning", "No projects available.", time() + (24*3600));
        header('Location: /');
    }
?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="container">
        ...
        <form action="newProjectProcess.php" method="POST" enctype="multipart/form-data">
            <div class="form-row">
                <div class="form-col">
                    <h2>Info</h2>
                    <div class="form-group select">
                        <select name="schluessel" id="schluessel">
                            <option value="" disabled selected>Key (foldername) *</option>
                            <?php 
                ...
                <div class="form-col">
                    <h2>File</h2>
                    <p>Select an existing file oder upload an image of your choice. The selected image overrides any upload. <br> If no file is selected or uploaded the default placeholder image will be set.</p>
                    <div class="form-group select">
                    ...
                    <input type="submit" name="submit" value="Add" class="btn btn-primary ml-auto">
                </div>
            </div>
